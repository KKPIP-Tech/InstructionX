...
框架语言文件位于 `ui/text/`（`zh.xml` 为默认语言文件，必须完整；另有 `en.xml` 英文、`ru.xml` 俄语与 `fr.xml` 法语）；插件语言文件位于各插件的 `text/` 目录。语言列表由该目录下的 `*.xml` 文件名自动发现（新增一个语言文件即多一个「语言」菜单选项），显示名取该文件 `common` 分组的 `language.self_name`。

### 2.1 新增一种语言

1. 复制 `zh.xml` 的**分组与键结构**，新建 `ui/text/<语言代码>.xml`（如 `ru.xml`、`fr.xml`），并把 `language.self_name` 写成本语言的自称（如「Русский」「Français」）；
2. 逐键翻译：占位符 `{name}` 与格式说明符（如 `{pct:+.1f}`）必须原样保留；计数类文案须规避该语言的数词变格／单复数变化（框架不提供复数规则），统一改写为「动词 + 名词复数第二格 + 数量」或「标签: 数值」结构（如 `Установлено плагинов: {count}`、`Plugins installés : {count}`）；
3. 运行 `scripts/check_i18n_completeness.py` 校验键集合与默认语言覆盖（其他语言缺键为警告、退出码 0）；
4. **版式复核（最容易出问题的一步）**：固定宽度控件不随语言自适应，必须让文案装进既有宽度，否则会被裁切。做法是在 `zh` 与目标语言下分别渲染各对话框，比较每个文本控件的需要宽度与实际宽度，只对「目标语言溢出、中文不溢出」的控件收紧文案。已知固定宽度处及实测结论：

   | 固定宽度处 | 宽度 | 处理方式（俄语 / 法语实测值） |
   |---|---|---|
   | 使用历史表格表头（`table.header.*`） | 列宽 70 / 80px | QSS 左右内边距各 10px 且表头以**粗体**绘制，实际可用宽度 ≈ 列宽 − 23px；故俄语用 «Сервис» / «Задерж.» / «Пот.» / «Кэш» / «Из кэша» / «Токенов»，法语用 «Service» / «Durée» / «Flux» / «Cache» / «Du cache» / «Tokens» |
   | 详情面板「检测」按钮 | 76px | 俄语 «Тест»（«Проверить» 需 100px）、法语 «Tester»（68px） |
   | 详情面板「重置」按钮 | 64px | 该宽度放不下任何完整词（俄语 «Сброс» 71px、法语 «Réinit.» 70px）：俄语用符号 «↺»，法语用通用缩写 «RAZ»，完整语义由 tooltip 承载 |
   | GitHub 安装对话框「检查」按钮 | 80px | 俄语 «Анализ»（77px）、法语 «Vérifier»（75px） |
   | 默认模型行标签 | 110px | 俄语 «Модель диалога» / «Эмбеддинги»；法语 «Modèle de chat» / «Embeddings» |
   | 分组编辑器上移/下移按钮 | 56px | 俄语 «Вверх» / «Вниз»；法语 «Haut» / «Bas»（«Descendre» 需 74px） |
   | KPI 卡片标题与同比小字 | 约 97px | 俄语 «Токены ввода» / «Кэш-попадания» / «к пред. {pct:+.1f}%»；法语 «Tokens d’entrée» / «Taux de cache» / «vs période {pct:+.1f} %» |

5. 术语与排版约定写进语言文件头部注释（缩写是否保留拉丁写法、按键用不定式、俄语引号 «…» 与数词变格规避、法语双标点前的不换行空格与 ’ 撇号等），便于后续维护者对齐风格。
...
flowchart TD
    ...
    F -->|插件提供| G[使用插件默认语言]
    F -->|均不可用| H["保底：插件实际提供的语言中<br/>按字典序排序后的首个"]
```
...
插件在自身目录下提供 `text/<语言代码>.xml`（结构同 §3.1），框架加载插件时由 `PluginManager` 自动扫描注册，**插件无需任何登记代码**；热卸载时自动注销。`text/` 目录为插件开发的**必需**项；框架对无 `text/` 目录的存量插件保持兼容，行为与旧版本完全一致。
...
#### 7.5.1 推荐实现：文案绑定表（不重建控件）

插件界面文案在构建期取词后即固化在控件上，因此"重取词"必须能定位到**具体控件**。
推荐做法是在插件内维护一张绑定表（参考实现：`plugin/robospine-control/ui/i18n_bind.py`
与另一工程 `plugin/framework_api_demo/ui/tabs/base_tab.py`）：

- 构建期登记：控件用取词结果创建后立刻登记 `(控件, setter, 分组, 键, 占位符参数)`，
  例如 `label = Label(self._tr("form.name")); self._bind(label, "form.name")`；
  非 `setText` 的目标用 `setter=`（分组标题 `setTitle`、占位符 `setPlaceholderText`、
  self-drawn 提示用自定义 setter）；
- 语言变化时按原键重设，**不重建任何控件**——滚动位置、输入内容、进行中的采集/
  记录状态全部保留；
- 容器页面把重设转发给子控件，并**隔离单点失败**（单个子控件抛异常只记 ERROR 日志，
  否则兄弟控件与兄弟页面会停在旧语言，界面处于半切换状态）；
- 运行期按状态取词的动态文案（状态行、读数、按状态切换的按钮文案）不入表，
  它们在下一次事件刷新时自然使用新语言，必要时在重设入口按当前状态重跑一次
  `_sync_*`。

#### 7.5.2 插件显示名（技能面板按钮）

技能面板按钮名取自 `IPlugin.plugin_name`。插件若希望名称本地化，应在该属性内经
`PluginServices.localization.tr(...)` 取词（如 RoboSpine 用插件语言包的
`common.panel_title`）；框架在语言变化后会刷新技能面板按钮：先比对各插件显示名
快照，**只有显示名确实变化才重建按钮**（固定品牌名的插件不受影响，不会重置分组
展开状态与焦点），重建时保留激活插件的高亮。插件无需为此做任何额外接线。
