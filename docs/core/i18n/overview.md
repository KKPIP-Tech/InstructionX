...
框架语言文件位于 `ui/text/`（`zh.xml` 为默认语言文件，必须完整；另有 `en.xml` 英文与 `ru.xml` 俄语）；插件语言文件位于各插件的 `text/` 目录。语言列表由该目录下的 `*.xml` 文件名自动发现（新增一个语言文件即多一个「语言」菜单选项），显示名取该文件 `common` 分组的 `language.self_name`。

### 2.1 新增一种语言

1. 复制 `zh.xml` 的**分组与键结构**，新建 `ui/text/<语言代码>.xml`（如 `ru.xml`），并把 `language.self_name` 写成本语言的自称（如「Русский」）；
2. 逐键翻译：占位符 `{name}` 与格式说明符（如 `{pct:+.1f}`）必须原样保留；计数类文案建议改写为「动词 + 名词第二格复数 + 数量」或「标签: 数值」结构（如 `Установлено плагинов: {count}`），以规避该语言的数词变格；
3. 运行 `scripts/check_i18n_completeness.py` 校验键集合与默认语言覆盖（其他语言缺键为警告、退出码 0）；
4. 版式复核：**固定宽度控件不随语言自适应**，必须让文案装进既有宽度，否则会被裁切。已知固定宽度处（俄语实测值）：使用历史表格列宽 70/80px（`table.header.*`）、详情面板「检测」按钮 76px、「重置」按钮 64px、GitHub 安装对话框「检查」按钮 80px、默认模型行标签 110px、分组编辑器按钮 56px；做法是在 `zh` 与目标语言下分别渲染各对话框，比较每个文本控件的 `sizeHint().width()` 与实际宽度，只对「目标语言溢出、中文不溢出」的控件收紧文案。
5. 术语与排版约定写进语言文件头部注释（缩写是否保留拉丁写法、按键用不定式、引号用 «…» 等），便于后续维护者对齐风格。
