            ...
            for (subscriber_id, target_plugin_id, target_key), callback in self._subscriptions.items():
                if target_plugin_id == publisher_id and target_key == key:
                    try:
                        ...
                        print(f"警告: 订阅者 {subscriber_id} 的回调函数执行失败: {e}")
...
def demo_callback(target_plugin_id: str, key: str, old_value: Any, new_value: Any):
    """演示用的回调函数"""
    print(f"📢 通知: 插件 '{target_plugin_id}' 的 '{key}' 从 '{old_value}' 变更为 '{new_value}'")


if __name__ == "__main__":
    print("=" * 60)
    print("DataProvider 演示程序")
    print("=" * 60)
    ...
    print(f"\n✅ 已注册插件 A: {plugin_a_id} (类型: VideoEditor)")
    
    # 注册插件 B（Exporter 类型）
    plugin_b_id = "exporter-001"
    provider.register_plugin(plugin_b_id, "Exporter")
    print(f"✅ 已注册插件 B: {plugin_b_id} (类型: Exporter)")
    
    # 设置插件 A 为活跃实例
    provider.set_active_instance(plugin_a_id)
    print(f"✅ 已将插件 A 设为 VideoEditor 类型的活跃实例")
    ...
    print(f"✅ 插件 B 订阅了插件 A 的 'video_duration' 和 'frame_rate' 变化")
    ...
    print(f"✅ 已保存资源文件: {relative_path}")
    
    # 获取资源路径
    absolute_path = provider.get_asset_path(relative_path)
    print(f"✅ 资源文件绝对路径: {absolute_path}")
    
    # 加载资源文件
    loaded_content = provider.load_asset(relative_path)
    print(f"✅ 已加载资源文件，内容: {loaded_content.decode()}")
    ...
    for pid, info in all_plugins.items():
        ...
        print(f"   活跃: {'是' if info['active'] else '否'}")
        print(f"   公共数据: {info['public']}")
        print(f"   私有数据: {info['private']}")
    ...
    print(f"✅ 已取消插件 B 的所有订阅")
    
    # 再次变更数据（不会触发通知）
    print("\n模拟再次变更数据（已取消订阅，不应触发通知）...")
