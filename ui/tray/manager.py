...
# ===================================================================
# 菜单与通知文案常量
TRAY_TOOLTIP = "InstructionX - CE"
MENU_SHOW_MAIN_WINDOW = "显示主窗口"
MENU_RUNNING_PLUGINS = "正在运行的插件"
MENU_RUNNING_TASKS = "后台正在运行的任务"
MENU_QUIT = "退出"
MENU_NO_PLUGINS = "无已加载插件"
MENU_NO_TASKS = "无运行中的任务"
NOTIFY_MINIMIZE_TITLE = "InstructionX - CE"
NOTIFY_MINIMIZE_MESSAGE = "程序已最小化到系统托盘，仍在后台继续运行"
...
class TrayIconManager(QObject):
    ...
    def show_minimize_hint(self) -> None:
        ...
        sent = self._backend.send_notification(
            self._tray, NOTIFY_MINIMIZE_TITLE, NOTIFY_MINIMIZE_MESSAGE
        )
        if not sent:
            self._logger.info(
                get_name(),
    ...
    def _build_menu(self) -> None:
        ...
        show_action = self._menu.addAction(MENU_SHOW_MAIN_WINDOW)
        show_action.triggered.connect(self.show_main_window_requested.emit)

        self._plugins_menu = self._menu.addMenu(MENU_RUNNING_PLUGINS)
        self._plugins_menu.aboutToShow.connect(self._rebuild_plugins_menu)

        self._tasks_menu = self._menu.addMenu(MENU_RUNNING_TASKS)
        self._tasks_menu.aboutToShow.connect(self._rebuild_tasks_menu)

        quit_action = self._menu.addAction(MENU_QUIT)
    ...
    def _rebuild_plugins_menu(self) -> None:
        ...
        if not entries:
            self._add_disabled_item(self._plugins_menu, MENU_NO_PLUGINS)
            return
    ...
    def _rebuild_tasks_menu(self) -> None:
        ...
        if not entries:
            self._add_disabled_item(self._tasks_menu, MENU_NO_TASKS)
            return
        for task_name, plugin_name in entries:
            self._tasks_menu.addAction(f"{task_name}（{plugin_name}）")
